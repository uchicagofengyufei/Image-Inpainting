...
def build_ae(input_var):
    ...
    l_decode = lasagne.layers.TransposedConv2DLayer(l_encode,num_filters=128,filter_size=(9,9),stride = 1,
    ...
    l_deconv_3 = lasagne.layers.TransposedConv2DLayer(l_deconv_2, num_filters=32, filter_size=(3, 3), crop="same",
                                                      stride=1,nonlinearity=lasagne.nonlinearities.rectify)
...
def build_discriminator(input_pic):

    l_in = lasagne.layers.InputLayer(shape=(None, 3, 36, 36),
                                     input_var=input_pic)
    ...
    l_pool_1 = lasagne.layers.MaxPool2DLayer(l_conv_1, pool_size=(2, 2), stride=2) #18
    #l_pool_1 = lasagne.layers.Pool2DLayer(l_conv_2, pool_size=(2, 2), stride=2,mode = 'average_inc_pad')

    l_conv_2 = lasagne.layers.Conv2DLayer(l_pool_1, num_filters=32, filter_size=(3, 3), pad='same',
                                          nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform(),
                                          b=None)

    l_pool_2 = lasagne.layers.MaxPool2DLayer(l_conv_2, pool_size=(2, 2), stride=2) #9
    #l_pool_2 = lasagne.layers.Pool2DLayer(l_conv_4, pool_size=(2, 2), stride=2,mode = 'average_inc_pad')

    l_conv_3 = lasagne.layers.Conv2DLayer(l_pool_2, num_filters=64, filter_size=(5, 5), pad='same',
                                          nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform(),
                                          b=None)

    l_pool_3 = lasagne.layers.MaxPool2DLayer(l_conv_3, pool_size=(2, 2), stride=2) #4
    #l_pool_3 = lasagne.layers.Pool2DLayer(l_conv_5, pool_size=(2, 2), stride=2,mode = 'average_inc_pad')

    #l_conv_6 = lasagne.layers.Conv2DLayer(l_pool_3, num_filters=64, filter_size=(6, 6),
                                          #nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform(),
                                          #b=None)

    #l_pool_4 = lasagne.layers.MaxPool2DLayer(l_conv_6, pool_size=(2, 2), stride=2)
    #l_pool_4 = lasagne.layers.Pool2DLayer(l_conv_6, pool_size=(2, 2), stride=2,mode = 'average_inc_pad')
...
adv_net = build_discriminator(sample_reconstruct)
...
combined_loss = 0.90*loss - 0.1*adv_loss
...
discriminator_fn = theano.function([sample_reconstruct,adv_target],adv_loss,updates = adv_updates)
...
num_epochs = 150
gen_label = numpy.zeros(500).astype(numpy.int32)
adv_label = numpy.zeros(1000).astype(numpy.int32)
adv_label[500:1000] = 1
for epoch in range(num_epochs):
    ...
    for batch in mini_batch.iterate_minibatches_pair(flower_corrupt_train,flower_truth_train, 500, shuffle=True):
        if epoch<50:
...
print(tloss)
recons_img = load_image.reconstruct(X_cor_view,rec_img)
...
for i in range(0,300):
    im_c = Image.fromarray(get_image_array(recons_img,i))
