...
def build_discriminator(input_pic):
    ...
    l_in = lasagne.layers.InputLayer(shape=(None, 3, 48, 48),
                                     input_var=input_pic)
    ...
    l_pool_1 = lasagne.layers.MaxPool2DLayer(l_conv_1, pool_size=(2, 2), stride=2) #24
    #l_pool_1 = lasagne.layers.Pool2DLayer(l_conv_2, pool_size=(2, 2), stride=2,mode = 'average_inc_pad')

    l_conv_2 = lasagne.layers.Conv2DLayer(l_pool_1, num_filters=32, filter_size=(5, 5), pad='same',
                                          nonlinearity=lasagne.nonlinearities.rectify, W=lasagne.init.GlorotUniform(),
                                          b=None)

    l_pool_2 = lasagne.layers.MaxPool2DLayer(l_conv_2, pool_size=(2, 2), stride=2) #12
    ...
    l_pool_3 = lasagne.layers.MaxPool2DLayer(l_conv_3, pool_size=(2, 2), stride=2) #6
...
combined_loss = 0.9*loss - 0.1*adv_loss
